...
*One of the two authentication headers is required. See [Authentication](/docs/api/authentication) for details.
...
```typescript
{
  ...
  weighted_contribution: number;   // component_score × weight
...
[See complete scoring methodology →](/docs/api/understand-responses/defi-score)
...
[See rate limits →](/docs/api/authentication#rate-limits)
...
- ✅ **30-second TTL** - Results cached for 30 seconds
- ✅ **Per-wallet cache key** - Each address cached separately
- ✅ **Check `X-Cache-Status` header** - Know if data is fresh or cached
- ✅ **`last_updated` timestamp** - Every position shows when it was fetched
...
<div className="button-group">
  <a href="(/docs/api/understand-responses/overview" className="button button--primary button--lg">
    🔍 Response Structure Deep-Dive
  </a>
  <a href="(/docs/api/understand-responses/defi-score" className="button button--secondary button--lg">
    🧮 How Scoring Works
  </a>
